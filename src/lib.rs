...
/// A [`NetworkBehaviour`] that responds to inbound msgs and
/// periodically sends outbound msgs on every established connection.
///
...
/// Event generated by the `msg` network behaviour.
#[derive(Debug)]
pub struct Event {
...
impl Behaviour {
    /// Creates a new `msg` network behaviour with the given configuration.
    pub fn new() -> Self {
        Self {
            events: VecDeque::new(),
